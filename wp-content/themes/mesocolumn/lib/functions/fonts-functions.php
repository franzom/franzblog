...
// get google web font family list
function dez_get_google_webfonts_family_list($key='', $sort='') {
...
// get google web font family list
function dez_get_google_webfonts($key='', $sort='') {
$http = (!empty($_SERVER['HTTPS'])) ? "https" : "http";
if(!$key):
$key = 'AIzaSyBmPRa5TGlBRbUUV-pVPU3GxXRkD4lBtUU';
else:
...
$google_api_url = 'https://www.googleapis.com/webfonts/v1/webfonts?key=' . $key . '&sort=' . $sort;
$font_list = get_transient( 'wp_mp_google_fonts_' . $sort );
...
set_transient( 'wp_mp_google_fonts_' . $sort, $font_list, 60 * 60 * 72 );
}
endif;
...
// get google web font family list in alphabetic
function dez_google_webfonts_family_list(){
$fontlist = dez_get_google_webfonts('AIzaSyBmPRa5TGlBRbUUV-pVPU3GxXRkD4lBtUU', 'alpha');
foreach($fontlist as $font): ?>
<?php echo '"'.ucfirst($font) . '",'; ?>
<?php endforeach;
}
//add_action('wp_head', 'dez_google_webfonts_family_list');
